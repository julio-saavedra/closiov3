...
export default function NeuralNetworkHero({
  ...
  useScrambleOnView(titleRef, title, {
    duration: 0.6,
    revealDelay: 0.03,
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-[#030712]">
      ...
      >
        ...
        <h1 ref={titleRef} className="max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl min-h-[120px] sm:min-h-[140px] md:min-h-[160px]">
          {title}
        </h1>
        ...
        <ul className="mt-8 flex flex-wrap gap-6 text-2xl font-extralight tracking-tight text-white/60">
          {microDetails.map((detail, index) => (
            <li key={index} className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-[#2C66FF]" /> {detail}
            </li>
          ))}
        </ul>
      </div>
